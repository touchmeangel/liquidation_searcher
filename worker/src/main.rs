...
#[derive(Clone)]
pub struct AssetNode<'a> {
  pub bank: &'a BankAccount,
  pub amount: I80F48,
  pub usd_value: I80F48,
}

#[derive(Clone)]
pub struct AssetToWithdraw<'a> {
  pub mint: Pubkey,
  pub amount: I80F48,
...
fn build_available_assets_map<'a>(user: &'a MarginfiUser) -> HashMap<Pubkey, AssetNode<'a>> {
  user.bank_accounts()
    .iter()
...
pub fn select_assets_to_withdraw<'a>(
  user: &'a MarginfiUser,
  swaps: Vec<&SwapPair>,
  target_usd: I80F48,
) -> anyhow::Result<Vec<AssetToWithdraw<'a>>> {
  let available = build_available_assets_map(user);
  let mut swap_totals: HashMap<Pubkey, (I80F48, I80F48)> = HashMap::new();

  for swap in swaps {
    let entry = swap_totals
      .entry(swap.from_mint)
      .or_insert((I80F48::ZERO, I80F48::ZERO));
    ...
    entry.1 = entry.1.checked_add(swap.from_amount_usd)
      .ok_or(anyhow::anyhow!("USD overflow"))?;
  }

  let mut candidates: Vec<AssetToWithdraw> = Vec::with_capacity(swap_totals.len());
  for (mint, (total_amount, total_usd)) in swap_totals {
    ...
    if total_usd > asset_node.usd_value {
      return Err(anyhow::anyhow!(
        ...
        total_usd, mint, asset_node.usd_value
      ));
    }

    candidates.push(AssetToWithdraw {
      bank: asset_node.bank,
      mint,
      amount: total_amount,
  ...
  for mut candidate in candidates {
    let remaining_needed = target_usd
      .checked_sub(accumulated_usd)
    ...
    let asset_node = available.get(&candidate.mint).unwrap();
    let max_additional_usd = asset_node.usd_value
      .checked_sub(candidate.amount_usd)
      .ok_or(anyhow::anyhow!("max additional overflow"))?;
    ...
    if max_additional_usd > I80F48::ZERO && remaining_needed > candidate.amount_usd {
      ...
      if candidate.amount > I80F48::ZERO {
        let price = candidate.amount_usd
          .checked_div(candidate.amount)
          .ok_or(anyhow::anyhow!("price calculation failed"))?;
        let additional_amount = additional_usd
          .checked_div(price)
          .ok_or(anyhow::anyhow!("amount calculation failed"))?;

        candidate.amount = candidate.amount
          .checked_add(additional_amount)
          .ok_or(anyhow::anyhow!("amount overflow"))?;
        candidate.amount_usd = candidate.amount_usd
    ...
    accumulated_usd = accumulated_usd
      .checked_add(candidate.amount_usd)
      .ok_or(anyhow::anyhow!("accumulated USD overflow"))?;
...
pub fn calculate_swap_pairs(user: &MarginfiUser) -> anyhow::Result<Vec<SwapPair>> {
  ...
  for (needed_mint, needed_asset) in &needed {
    let already_covered: I80F48 = swaps.iter()
      .filter(|s| s.to_mint == *needed_mint && s.from_mint == *needed_mint)
      .map(|s| s.from_amount)
      .sum();

    let remaining_needed_amount = needed_asset.amount - already_covered;
    if remaining_needed_amount <= I80F48::ZERO {
      continue;
    }

    if needed_asset.amount <= I80F48::ZERO {
      continue;
    }
    let unit_price = needed_asset.usd_value / needed_asset.amount;
    let usd_value_needed = remaining_needed_amount * unit_price;

    let mut sorted_keys: Vec<Pubkey> = available
      .iter()
      .filter(|(k, v)| *k != needed_mint && v.usd_value > I80F48::ZERO)
      .map(|(k, _)| *k)  // Copy
      .collect();

    sorted_keys.sort_by(|a, b| available[b].usd_value.cmp(&available[a].usd_value));

    let mut remaining_value = usd_value_needed;

    for available_mint in sorted_keys {
      if remaining_value <= I80F48::ZERO {
        break;
      }

      let available_asset = available.get_mut(&available_mint).unwrap();

      if available_asset.usd_value <= I80F48::ZERO || available_asset.amount <= I80F48::ZERO {
        continue;
      }
      ...
      swaps.push(SwapPair {
        from_mint: available_mint,
        to_mint: *needed_mint,
        from_amount: amount_to_swap,
        from_amount_usd: value_to_use,
    ...
    if remaining_value > I80F48::from_num(0.01) {
      anyhow::bail!(
        "Insufficient funds to cover {} (${:.2} short)",
        needed_mint, remaining_value
      );
    }
  }
...
pub async fn build_liquidation_tx<'a>(
  rpc_client: &RpcClient,
  user: &MarginfiUser,
  fee_state: &FeeState,
  payer: &Keypair,
  assets_to_withdraw: Vec<(AssetToWithdraw<'a>, Account)>,
  ...
  let lookup_tables: Vec<AddressLookupTableAccount> = swap_responses
    ...
    .fold(HashMap::new(), |mut map, (key, addresses)| {
      map.entry(*key).or_insert(addresses);  // key is Pubkey (Copy)
      map
    })
    .into_iter()
    .map(|(key, addresses)| AddressLookupTableAccount { key, addresses: addresses.clone() })
    .collect();
...
fn build_liquidation_instructions(
  ...
  let mut out_mint_grouped: HashMap<Pubkey, (Account, BankAccount, I80F48)> = HashMap::new();
  for (pair, account, expected_output) in assets_to_repay {
    out_mint_grouped
      .entry(pair.to_mint)
      .or_insert((account, pair.to_bank_account, I80F48::ZERO))
      .2 += expected_output;
  }

  for (mint, (mint_account, to_bank_account, expected_output)) in out_mint_grouped {
    let token_program = mint_account.owner;
  ...
  for swap in swap_responses {
    ...
    instructions.extend(swap.other_instructions.iter().cloned());
